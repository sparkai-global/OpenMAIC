...
OpenMAIC 内置了 [OpenClaw](https://github.com/anthropics/openclaw) 插件，让任何 OpenClaw 驱动的 AI 智能体都能通过自然语言管理和生成互动课堂。

### 安装

```bash
# 在你的 OpenClaw 项目中，从 openclaw/ 目录安装插件：
openclaw plugins install /path/to/OpenMAIC/openclaw
```

### 配置

在 OpenClaw 项目的 `~/.openclaw/openclaw.json` 中更新插件配置：
...
```jsonc
{
  "plugins": {
    "entries": {
        "openmaic": {
        "url": "http://localhost:3000",       // OpenMAIC 服务地址
        "projectDir": "/path/to/OpenMAIC"     // 项目目录（用于启动/停止/安装依赖）
      }
    }
  }
}
...
| 工具 | 说明 |
|------|------|
| `openmaic_manage` | 启动、停止、检查状态或安装 OpenMAIC 服务依赖 |
| `openmaic_generate` | 根据自由文本需求和/或 PDF 文件生成完整的互动课堂 |

### 使用示例

```
你：「生成一个面向高中生的量子力学课堂」
智能体：调用 openmaic_generate { requirement: "面向高中生的量子力学课堂" }
智能体：「课堂已生成！地址：http://localhost:3000/classroom/abc123」
```

```
你：「用这个 PDF 做一节课」（附带 /path/to/paper.pdf）
智能体：调用 openmaic_generate { requirement: "基于 PDF 材料生成课堂", pdfPath: "/path/to/paper.pdf" }
```

智能体还可以管理服务器生命周期——部署、启动、停止——全部通过自然语言完成。
...
│   ├── openclaw.plugin.json    #   插件清单（配置项、技能定义）
│   ├── index.ts                #   插件入口——注册工具
│   ├── src/tools.ts            #   工具实现（管理 & 生成）
│   └── skills/openmaic/        #   AI 智能体技能定义
│
