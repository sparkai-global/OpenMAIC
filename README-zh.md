...
OpenMAIC 内置了一个 [OpenClaw](https://github.com/openclaw/openclaw) skill，用来以“逐步确认”的 SOP 方式引导用户完成 clone、启动方式选择、Provider Key 配置，以及课堂生成。
...
| Provider Key | 推荐配置路径，并引导用户自己编辑 `.env.local` 或 `server-providers.yml` |
| 生成 | 在读取 PDF 前确认，提交异步生成任务，然后轮询同一个 job，或提示用户稍后回来继续查询 |
...
```
你：「根据这个 PDF 生成课堂」
智能体：「OpenMAIC 服务已经健康。我需要先读取 PDF。之后我会提交课堂生成任务，并继续帮你跟进进度。」
```
